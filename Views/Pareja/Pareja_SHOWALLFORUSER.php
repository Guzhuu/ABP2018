			...
			while($fila = $this->resultado->fetch_row()){
				...
				for($i = 0; $i < sizeof($fila); $i++){
					...
					echo "<input type='hidden' name='"; echo $this->resultado->fetch_fields()[$i]->name; echo "' value=\""; echo $fila[$i]; echo "\">";
	 				...
	 				echo '<input type="hidden" name="codPareja" value="'; echo $fila[0]; echo '"/>';
					echo $fila[$i];
					echo '</td>';
				}
				...
				if($this->showAcciones){
					echo '<td class="celda">';
						...
						if($this->showInscribirse){
							echo "<input class='btn btn-primary' type='submit' name='submit' value='VERCAMPEONATOS'/>";
						}
						$this->botonesOpcion();
					echo '</td>';
				}
